...
void setup(void)
{
  ...
  initialize_procedures();
	...
	//register_event(EVENT_CALCULATING_EULER_ANGLES, Euler_Angles);
        register_event(EVENT_STABILISATION, stabilisation);
	...
	// TIMERS
        add_timer(EVENT_STABILISATION, FREQUENCY_STABILISATION);
...
void display_sensors_values(void)
{
  ...
  return;
	...
	char buf[120];
	sprintf(buf, "acc:\tX:%d\tY:%d\tZ:%d\t\tgyro:\tX:%d\tY:%d\tZ:%d\t\tmagn:\tX:%d\tY:%d\tZ:%d", accelX, accelY, accelZ, gyroX, gyroY, gyroZ, magnX, magnY, magnZ);
	Serial.println(buf);
}
